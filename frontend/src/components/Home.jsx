...
function Home() {
  ...
  useEffect(() => {
    const fetchtodos = async () => {
      try {
        setLoading(true);
        const response = await axios.get("https://todo-application-er6p.vercel.app/todo/fetch", {
  ...
  const todoCreate = async () => {
    ...
    try {
      const response = await axios.post(
        "https://todo-application-er6p.vercel.app/todo/create",
        {
          text: newTodo,
          completed: false,
        },
  ...
  const todoStatus = async (id) => {
    ...
    try {
      const response = await axios.put(
        `https://todo-application-er6p.vercel.app/todo/update/${id}`,
        {
          ...todo,
  ...
  const todoDelete = async (id) => {
    try {
      await axios.delete(`https://todo-application-er6p.vercel.app/todo/delete/${id}`, {
        withCredentials: true,
      });
  ...
  const logout = async () => {
    try {
      await axios.get("https://todo-application-er6p.vercel.app/user/logout", {
        withCredentials: true,
      });
